...
1.  **`/content`**: A content folder holding assets that the timeline theme expect to exist. Check out the README for each theme to learn more about the demo content.

2.  **`/src`**: You will probably want to customize your site to personalize it. The files under `/src/gatsby-theme-timeline` and `/src/gatsby-theme-notes` _shadow_, or override the files of the same name in their respective packages. To learn more about this, check out the [guide to getting started with using the timeline theme starter](https://gatsbyjs.com/docs/themes/using-a-gatsby-theme).
